...
class show:
    ...
    def __init__(self):
        ...
        Button(self.frm_MB, text="清除", command=self.clear, width=6, height=1, font=('Arial', 10)).pack(side=LEFT)
        Button(self.frm_MB, text="查询", command=self.search, width=6, height=1, font=('Arial', 10)).pack(side=RIGHT)
    ...
    def search(self):
        ...
        if tmp_char != '':
            if not self.char_info_hash.has_key(tmp_char):
                ...
                self.t_show.insert('1.0', '十六进制:' + '\t' + self.char_info_hash[tmp_char][2] + '\n')
                self.t_show.insert('1.0', '十进制:' +'\t' + self.char_info_hash[tmp_char][1] + '\n')
                self.t_show.insert('1.0', '八进制:' + '\t' + self.char_info_hash[tmp_char][0] + '\n')
            ...
            if not self.num_info_hash.has_key(tmp_int):
                ...
                self.t_show.insert('1.0', '字符:' +'\t' + self.num_info_hash[tmp_int][2] + '\n')
                self.t_show.insert('1.0', '十六进制:' + '\t' + self.num_info_hash[tmp_int][1] + '\n')
                self.t_show.insert('1.0', '八进制:' + '\t' + self.num_info_hash[tmp_int][0] + '\n')
