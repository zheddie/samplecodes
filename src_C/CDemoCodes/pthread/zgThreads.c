...
void * MonitorThread(void * p){
	while(!mainExit){
		...
		printf("zg.MONITOR.created=%d,ended=%d\n",createdThreads,endedThreads);
	}
}
...
void * CreatorThread( void * parm){
	...
	for(int k=0;k<workNum;k++){
		...
		if(rc){
			printf("zg.CreatorThread.ERROR.thread create failed.rc=%d\n",rc);
		}
		//sleep(1);
...
int main(int argc, char *argv[])
{
	...
	printf("zg.MAIN.creatorNum=%d,workNum=%d,speedNum=%d,maxWaitTime=%d\n",creatorNum,workNum,speedNum,maxWaitTime);
