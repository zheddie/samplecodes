 ...
 int main(int argc, char *argv[]) {
    ...
    JDK1_1InitArgs vm_args;
     ...
     /* Create the Java VM */
      ...
      printf("cp1,argv[1]=%s,argv[2]=%s\n",argv[1],argv[2]);
     res = JNI_CreateJavaVM(&jvm, &env, &vm_args);
	 	 
 #endif /* JNI_VERSION_1_2 */
 	   if(res){
 	   		printf("cp3,Create JVM failed %d\n",res);
 	   		return 0;
 	   }
    ...
    if (! myClass) {
       printf("cp4,Failed to find class");
      return 0;
    }
    ...
    if (! mainID) {
      printf("cp5,Failed to find jmethodID of 'main'");
      return 0;
    }
    ...
    if (! stringClass) {
       printf("cp6,Failed to find java/lang/String\n");
       return -1;
    }
    ...
    if (! argstr) {
       printf( "cp7,Failed to create args array\n");
       return (-1);
    }
