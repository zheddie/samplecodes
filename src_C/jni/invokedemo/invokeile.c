...
int convert(int oldccsid,int newccsid, char* olds, unsigned int oldlen, char **news, unsigned int* newlen){
 ...
 int main(int argc, char *argv[]) {
     ...
     jclass stringClass;
 	   jobjectArray argstr; 
     jclass  myClass;  
     char strStartPase[100];
	   int inp=0;
     int is64 = 0;
	   char *arg819[3];
	   unsigned int len[3];
	   char * env_java_home = getenv("JAVA_HOME");
     memset(strStartPase,0,100);
     if(argc>=4 && strcmp(argv[3],"64")==0){
       is64 = 1;
     }

     if(env_java_home){
       printf("zg.JAVA_HOME=%s\n",env_java_home);
       char * bits = env_java_home+strlen(env_java_home)-6;
       if(strstr(bits,"64bit") != NULL){
         is64=1;
        //  #sprintf(strStartPase , "%s/jre/lib/ppc64/jvmStartPase",env_java_home);
      //  }else{
      //   //  sprintf(strStartPase , "%s/jre/lib/ppc/jvmStartPase",env_java_home);
       }
     }
    if(is64){
      strcpy(strStartPase , "/usr/lib/start64");
    }else{
      strcpy(strStartPase , "/usr/lib/start32");
    }
     ...
     vm_args.ignoreUnrecognized = JNI_FALSE;
	   ...
	   /* Create the Java VM */
	     // Qp2EndPase();
    if (Qp2ptrsize() != 0){ printf("pase already exist!\n");}
   else {
     printf("NO pase available\nTrying to start pase with %s\n",strStartPase);
      QP2SHELL2(strStartPase);
      }
    ...
    if (Qp2ptrsize() != 0){ 
     ...
     printf("ERROR:pase not started.Exit....\n");
   }
