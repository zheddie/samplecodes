...
void d4_setzero(float **** mat,int c0,int c1,int c2,int c3){
	int i=0,j=0,k=0,l=0;
	for(i=0;i<c0;i++){
		for(j=0;j<c1;j++){
			for(k=0;k<c2;k++){
...
float * d1_zeroconstruct(int c0){
	int i=0,j=0;
	float * mat = NULL;
	mat = (float *)malloc(sizeof(float )*c0);
	memset(mat,0,sizeof(float )* c0);
	return(mat);
}
...
float **** conv2d_im2col(float **** out,float ****img,float ****flt,int img_c0,int img_c1,int img_c2,int img_c3,int flt_c0,int flt_c1,int flt_c2,int out_h,int out_w,float ** om,float ** fm,int om_h,int om_w,int fm_h,int fm_w){
	...
	for(c0=0;c0<img_c0;c0++){
		...
		for(s0=0;s0<om_w;s0++){
			for(c1=0;c1<img_c1;c1++){
				flt_c1_max = flt_c1;
				if(flt_c1 > img_c2-s0/out_w){/*others are zeros, do not need to do*/
					flt_c1_max = img_c2 -s0/out_w;
				}
				for(c2=0;c2<flt_c1_max;c2++){
					flt_c2_max = flt_c2;
					if(flt_c2 > img_c3-s0%out_w){/*others are zeros, do not need to do*/
						flt_c2_max = img_c3 -s0%out_w;
					}
					for(c3=0;c3<flt_c2_max;c3++){
						om[c1*flt_count_per_feature+c2*flt_c1+c3][s0] = img[c0][c1][s0/out_w+c2][s0%out_w+c3];
						//tmp[s1]+=img[c0][c1][s0/out_w+c2][s0%out_w+c3] * fm[s1][c1*flt_count_per_feature+c2*flt_c1+c3]
					}
				}
			}
			...
			for(s1=0;s1<fm_h;s1++){
				float tmp = 0.;
				for(s2=0;s2<fm_w;s2++){
					tmp+= fm[s1][s2]*om[s2][s0];
				}
				out[c0][s1][s0/out_w][s0%out_w]=tmp;
			}
		}
		...
		// 		}
		// 		out[c0][s1][s2/out_w][s2%out_w]=tmp;
		// 	}
		// }
	}
...
float **** conv2d_im2colv2(float **** out,float ****img,float ****flt,int img_c0,int img_c1,int img_c2,int img_c3,int flt_c0,int flt_c1,int flt_c2,int out_h,int out_w,float **om,float ** fm){
	...
	if(VERIFY) d2_print("fm",fm,flt_c0,flt_c1*flt_c2*img_c1);
	int om_w = out_h*out_w;
	for(c0=0;c0<img_c0;c0++){
		/*go through each image*/
		/*try to transform each image*/
		
		for(s0=0;s0<om_w;s0++){
			for(s1=0;s1<flt_c0;s1++){
				for(c1=0;c1<img_c1;c1++){
					flt_c1_max = flt_c1;
					if(flt_c1 > img_c2-s0/out_w){/*others are zeros, do not need to do*/
						flt_c1_max = img_c2 -s0/out_w;
					...
					for(c2=0;c2<flt_c1_max;c2++){
						...
						for(c3=0;c3<flt_c2_max;c3++){
							...
							om[c1*flt_count_per_feature+c2*flt_c1+c3][s0] = img[c0][c1][s0/out_w+c2][s0%out_w+c3];
							// out[c0][s1][s0/out_w][s0%out_w]+=img[c0][c1][s0/out_w+c2][s0%out_w+c3] * fm[s1][s2];
							// if(VERIFY) printf("out[%d,%d,%d,%d](%.2f)+=img[%d,%d,%d,%d](%.2f)*fm[%d,%d](%.2f)\n"
							// 	,c0,s1,s0/out_w,s0%out_w,out[c0][s1][s0/out_w][s0%out_w]
							// 	,c0,c1,s0/out_w+c2,s0%out_w+c3,img[c0][c1][s0/out_w+c2][s0%out_w+c3]
							// 	,s1,s2,fm[s1][s2]);
							//tmpspace[s1] +=img[c0][c1][s0/out_w+c2][s0%out_w+c3] * fm[s1][s2];
						}
					}
				}
			}
			...
			for(s1=0;s1<flt_c0;s1++){
				float tmp = 0.;
				for(s2=0;s2<flt_c1*flt_c2*img_c1;s2++){
					tmp+= fm[s1][s2]*om[s2][s0];
				}
				out[c0][s1][s2/out_w][s2%out_w]=tmp;
			}
...
float **** conv2d_im2colv3(float **** out,float ****img,float ****flt,int img_c0,int img_c1,int img_c2,int img_c3,int flt_c0,int flt_c1,int flt_c2,int out_h,int out_w,float ** om,float ** fm){

	int c0=0,c1=0,c2=0,c3=0;
	int s0=0,s1=0,s2=0,s3=0;
	int flt_c1_max = 0, flt_c2_max = 0;
	int fm_w = flt_c1*flt_c2*img_c1;
	int om_w = out_h*out_w;
	/*try to transform filter firstly*/
	int flt_count_per_feature = flt_c1*flt_c2;
	for(c0=0;c0<flt_c0;c0++){
	...
	if(VERIFY) d2_print("fm",fm,flt_c0,fm_w);
	for(c0=0;c0<img_c0;c0++){
		/*go through each image*/
		/*try to transform each image*/
...
conv2d_im2col:
16*40*3*3+40*(49*3*3*3*(2+2+4+1)(/,+,%,+,*,+,*,+,=)+16*49*9*3*(3)(*,+,=)+16*49*(3)(/,%,=)) = 3114280
Increased in theory: (3386880-3114280)/3386880=8.049%
*/
/*
➜  cudnn ./conv2d    10 times .                  
Timeconsumed(old): 60614607/1000000
Timeconsumed(new): 43245614/1000000
Increase(%):28.654797

➜  cudnn ./conv2d                                
Timeconsumed(old): 60148712/1000000
Timeconsumed(new): 34281359/1000000
Increase(%):43.005665

➜  cudnn ./conv2d
Timeconsumed(old): 60796803/1000000
Timeconsumed(new): 35567035/1000000
Increase(%):41.498508
...
/*Try to verify the method*/
		...
		//tmpspace = d1_zeroconstruct(fm_h);
		//d4_setzero(convout,img_count,flt_count,conv_h,conv_w);
		//conv2d_im2colv2(convout,images,filter,img_count,img_chl,img_h,img_w,flt_count,flt_h,flt_w,conv_h,conv_w,om,fm);
		...
		conv2d_im2colv3(convout,images,filter,img_count,img_chl,img_h,img_w,flt_count,flt_h,flt_w,conv_h,conv_w,om,fm);
