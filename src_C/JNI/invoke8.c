...
int convert(int oldccsid,int newccsid, char* olds, unsigned int oldlen, char **news, unsigned int* newlen){
 ...
 int main(int argc, char *argv[]) {
     ...
     if(convert(0,1208,argv[1],strlen(argv[1]),&arg1208[1],&len[1]) || convert(0,1208,argv[2],strlen(argv[2]),&arg1208[2],&len[2])){
	    	printf("Failed to convert args\n");
	    	return -1;
	    }
     options[0].optionString =arg1208[1];
...
#pragma convert(1208)
    myClass = (*env)->FindClass(env,arg1208[2]);
#pragma convert(0)
    if (! myClass) {
