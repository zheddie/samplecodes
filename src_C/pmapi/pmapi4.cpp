...
#include <string.h>
#include <dlfcn.h>
#include "PPCHWProfilerPrivate.hpp"
...
static bool pmapiInit()
   {
   	printf("zg.pmapiInit.cp0\n");
   if (pmapi.dlHandle)
      return true;
printf("zg.pmapiInit.cp1\n");
#ifdef TR_HOST_64BIT
   ...
   if (!pmapi.dlHandle)
      {
      printf("Failed to open libpmapi.");
      goto fail;
      }
printf("zg.pmapiInit.cp2\n");
   char *curSym;
...
static bool pmapiInitPM()
   ...
   if (rc = pmapi.pm_initialize(filter | PM_GET_GROUPS, &pmapi.pminfo, &pmapi.pmgroups, PM_CURRENT))
      {
      printf("Failed to initialize PMAPI, rc: %d, %s", rc, pmapi.pm_strerror("pm_initialize", rc));
      goto fail;
      }
printf("zg.pmapiInitPM.cp1\n");
   for (int i = 0; i < NumProfilingConfigs; ++i)
      {
      int group = findEventGroup(pmapi.pminfo, pmapi.pmgroups, configs[i].config, filter);
      if (group < 0)
         {
         printf("Config %u: Could not find a group containing the requested events.", i);
         goto fail;
         }
      configs[i].group = group;
printf("zg.pmapiInitPM.cp2\n");
      printf("Config %u mapped to group %u:", i, group);
      for (int c = 0; c < MAX_PMCS; ++c)
         {
         const pm_events2_t *event = &pmapi.pminfo.list_events[c][pmapi.pmgroups.event_groups[group].events[c]];
         printf("PMC[%u]: %s (%s)", c + 1, event->short_name, event->status.b.verified ? "verified" : event->status.b.unverified ? "unverified" : event->status.b.caveat ? "caveat" : "unknown status");
         }
      }
printf("zg.pmapiInitPM.cp3\n");
   return true;
...
initializeThread()
   ...
   if (IS_THREAD_RI_INITIALIZED(vmThread))
      {
      printf("J9VMThread=%p initialized for HW profiling.", vmThread);
      return true;
      }
#else
   ...
   printf("Initializing PMU on J9VMThread=%p, tid=%u.", vmThread, thread_self());
#else /* !(DEBUG || PPCRI_VERBOSE) */
   printf("Initializing PMU on J9VMThread=%p.", vmThread);
   ...
   if (rc != 0 && rc != 59)
      {
      printf("Failed to set PMAPI program for J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_set_program_mythread", rc));
      goto fail;
      }
   ...
   if (rc == 59)
      {
      printf("PMAPI program already exists for J9VMThread=%p.");
      rc = pmapi.pm_stop_mythread();
      // If the return code is 85, counting was never started on the thread and we can still proceed
      if (rc != 0 && rc != 85)
         {
         printf("Failed to stop counting on J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_stop_mythread", rc));
         goto fail;
         }
      if (rc = pmapi.pm_delete_program_mythread())
         {
         printf("Failed to delete PMAPI program for J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_delete_program_mythread", rc));
         goto fail;
         }
   ...
   for (int i = 0; i < MAX_PMCS; ++i)
      ...
      if (configs[currentConfig].sampleRates[i] == INVALID_SAMPLERATE)
         continue;
      bufs[i] = (void *)malloc(configs[currentConfig].bufferElementSize[i] * configs[currentConfig].bufferSize[i]);
      if (!bufs[i])
         {
         printf("Failed to allocate buffer %u for J9VMThread=%p.", i + 1, vmThread);
         // Don't have enough memory now, but might in the future, so don't disable HWP completely
         setUnavailableOnFail = false;
         goto freebufs;
         }
      printf("Allocated initial buffer %p for J9VMThread=%p.", bufs[i], vmThread);
      }

   context = (TR_PPCHWProfilerEBBContext*)malloc(sizeof(TR_PPCHWProfilerEBBContext));
   if (!context)
      {
      printf("Failed to allocate context for J9VMThread=%p.", vmThread);
      goto freebufs;
      }
   printf("Allocated context=%p for J9VMThread=%p.", context, vmThread);

   context->eventHandler = configs[currentConfig].eventHandler;
   context->vmThread = (J9VMThread*)vmThread;
   context->currentConfig = currentConfig;
   for (int i = 0; i < MAX_PMCS; ++i)
      {
      context->eventCounts[i] = 0;
      context->sampleRates[i] = (configs[currentConfig].sampleRates[i] == INVALID_SAMPLERATE ? INVALID_SAMPLERATE : 500000);
   ...
   if (rc = pmapi.pm_set_counter_frequency_mythread(sampleRates))
      {
      printf("Failed to set PMU counter frequencies for J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_set_counter_frequency_mythread", rc));
      goto freecontext;
      }
   // if (rc = pmapi.pm_set_ebb_handler(FUNCTION_ADDRESS(ebbHandler), context))
   //    {
   //    printf("Failed to set EBB handler for J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_set_ebb_handler", rc));
   //    goto freecontext;
   //    }
   if (rc = pmapi.pm_enable_bhrb(configs[currentConfig].bhrbFilter))
      {
      printf("Failed to enable BHRB for J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_enable_bhrb", rc));
      goto clearebbh;
      }
   if (rc = pmapi.pm_start_mythread())
      {
      printf("Failed to start PMU counting on J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_start_mythread", rc));
      goto disablebhrb;
      }
   ...
   return true;
stopcounting:
   printf("Lost PMU access during initialization on J9VMThread=%p, will retry later.", vmThread);
   if (rc = pmapi.pm_stop_mythread())
      printf("Failed to stop counting on J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_stop_mythread", rc));
disablebhrb:
   if (rc = pmapi.pm_disable_bhrb())
      printf("Failed to disable BHRB for J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_delete_program_mythread", rc));
clearebbh:
   if (rc = pmapi.pm_clear_ebb_handler(NULL, NULL))
      printf("Failed to clear EBB handler for J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_delete_program_mythread", rc));
freecontext:
   free(context);
freebufs:
   for (int i = 0; i < MAX_PMCS; ++i)
...
fail:
   ...
   if (setUnavailableOnFail)
      {
      printf("Failure on J9VMThread=%p was critical. HW profiling will be unavailable from now on.", vmThread);
...
bool
deinitializeThread(J9VMThread *vmThread)
   {
   ...
   if (rc = pmapi.pm_clear_ebb_handler(NULL, (void**)&context))
      printf("Failed to clear EBB handler for J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_clear_ebb_handler", rc));
   else
      printf("Retrieved context=%p for terminating J9VMThread=%p.", context, vmThread);
   for (int i = 0; i < MAX_PMCS; ++i)
      {
#if defined(DEBUG) || defined(PPCRI_VERBOSE)
      printf("\tPMC[%u] count: %u", i + 1, context->eventCounts[i]);
#endif /* DEBUG || PPCRI_VERBOSE */
      if (context->buffers[i].start)
         free(context->buffers[i].start);
      }
#if defined(DEBUG) || defined(PPCRI_VERBOSE)
   printf("\tUnknown events: %u", context->unknownEventCount);
#endif /* DEBUG || PPCRI_VERBOSE */
   free(context);
   if (rc = pmapi.pm_delete_program_mythread())
      printf("Failed to delete PMAPI program for J9VMThread=%p, rc: %d, %s", vmThread, rc, pmapi.pm_strerror("pm_delete_program_mythread", rc));
...
int main()
{
	...
	bool icall = false;
	if(rt != 0){
		printf("allocation failure:rt=%d\n",rt);
		return(rt);
	}else{
		printf("allocation success\n");
		
	}
	icall = initializeThread();
	if(icall){
